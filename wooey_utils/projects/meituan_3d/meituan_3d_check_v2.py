# -*- coding: utf-8 -*-
# @Author : Thomas
# @File : meituan_3d_check_v2.py
# @CreateTime : 2021/7/15 1:31 下午
# @Project:
# @Description :

# Henry Wang, 2021-05-17 12:53 Changed:
# pip install mayavi


from . import perception_3d_label_pb2
...
CLS = str(
    "pedestrian,deformed_pedestrian,elderly_mobility_scooter,minibus,car,bigcar,engineering_vehicle,cyclist,motorlist,tricycle_rider,cart,other_cart,tricycle,low_barrier,cone_bucket,water_filled_barrie,anti_collision_bucket,warning_sign,construction_sign,other_unknown"
).split(",")
ALL_CLS = str(
    "pedestrian,deformed_pedestrian,elderly_mobility_scooter,minibus,car,bigcar,engineering_vehicle,cyclist,motorlist,tricycle_rider,cart,other_cart,tricycle,low_barrier,noise,cone_bucket,water_filled_barrier,anti_collision_bucket,warning_sign,construction_sign,other_unknown"
).split(",")
RIGID_CLS = str(
...
LOC_MAX_STD_VAR = 0.2
NOISE_BOX_MIN_Z = 2.5
TRACK_MAX_ANGLE_DIFF = 90
TRACK_MIN_MOVE = 2
STATIC_LOC_MAX_STD_VAR = 0.2
...
def check_anno_num(labeled_sequence, seq_result_dict, seq_error_dict):
    ...
    for labeled_frame in labeled_frames:
        ...
        labeled_boxes = [
            ...
            if perception_3d_label_pb2.ClassName.Name(
        ...
        for labeled_box in labeled_boxes:
            ...
            # check class name
            box_cls_name = perception_3d_label_pb2.ClassName.Name(
                box_info.class_name)
            ...
            if (box_cls_name.lower() in CLS
                ...
                box_motion_state = perception_3d_label_pb2.MotionState.Name(
                ...
                if box_motion_state == "STATIONARY":
                    ...
                    if box_tracking_id in static_tracking_id_dict:
                        if len(static_tracking_id_dict[box_tracking_id]) == 1:
                            seq_result_dict["CriticalError"].append(
                                "motion state: box tracking id: %d, motion state MOVING after short STATIONARY for class :%s"
                                % (box_tracking_id, box_cls_name))
                        static_tracking_id_dict[box_tracking_id].clear()
                        static_lidar_to_world_dict[box_tracking_id].clear()
    ...
    for labeled_frame in labeled_sequence.frames:
        ...
        for box in noise_boxes:
            ...
            if box_bottom_z > NOISE_BOX_MIN_Z:
                ...
                seq_error_dict[error_key] = 1
        # check iou
        # bbox_2d = get_bbox2d_from_bbox3d(labeled_boxes)
...
def check_seq_stability(tracking_id_dict, tracking_id_index_dict,
    ...
    for key, value in tracking_id_dict.items():
        ...
        cls = perception_3d_label_pb2.ClassName.Name(
            value[0].box3d_info.class_name)
        if cls.lower() not in CLS or cls.lower() in ["deformed_pedestrian", "other_unknown"]:
            continue
        for bbox in value:
...
def check_global_position_diff(boxes, lidar_to_worlds, max_std_diff):
    ...
    if x_var > max_std_diff or y_var > max_std_diff:
        check_var = dict()
        if x_var >LOC_MAX_STD_VAR:
            check_var["x"] =list(position_x)
        if y_var >LOC_MAX_STD_VAR:
            check_var["y"] =list(position_y)
...
def check_static_box_stability(static_tracking_id_dict,
    ...
    for key, value in static_tracking_id_dict.items():
        ...
        if len(value) > 1:
            flag, var_dict = check_global_position_diff(value, lidar_to_world,
                                              LOC_MAX_STD_VAR)
            if not flag:
                seq_result_dict["Error"].append(
                    ...
                    % key)
                # seq_result_dict["Error"].append(
                #     f"Critical ERROR: tracking_id {key}, static box location differ exceed the limit :{repr(var_dict)}\n")
    return seq_result_dict
...
def check_sequence(labeled_sequence, seq_result_dict, seq_error_dict):
    ...
    seq_result_dict["Info"][
        "TimePeriod"] = perception_3d_label_pb2.TimePeriod.Name(
    ...
    (total_bbox, each_frame_bbox, each_frame_index, class_count_dict,
     ...
     seq_result_dict) = check_anno_num(labeled_sequence, seq_result_dict,
                                       seq_error_dict)
...
# def meituan_3d_check_main_func(labeled_sequence_proto, error_log_file, batch_dir, check_point=False):
def meituan_3d_check_main_func(labeled_data_dir, error_log_file, visualize=False, replace_path=None):
    ...
    for batch_dir in sorted(os.listdir(labeled_data_dir)):
        ...
        og_batch_dir = os.path.join(labeled_data_dir, batch_dir) #batch_dir  ...+pc_20210511_7961
        other_batch_dir = os.path.join(replace_path,batch_dir)

        for record_dir in sorted(os.listdir(og_batch_dir)):
            if record_dir == ".DS_Store":
                continue
            record_dir = os.path.join(og_batch_dir, record_dir)
            for seq_task_dir in sorted(
                ...
                labeled_sequence = perception_3d_label_pb2.LabeledSequence()
