...
from labeling_proto_v2 import perception_3d_label_pb2 as perception_3d_label_pb2_v32
...
MIN_DYNAMIC_TRACK_POSITION_DIFF = 0.01
...
def check_anno_num(labeled_sequence, seq_result_dict, seq_error_dict):
    ...
    for labeled_frame in labeled_frames:
        ...
        labeled_boxes = [
            ...
            if perception_3d_label_pb2_v32.ClassName.Name(
        ...
        for labeled_box in labeled_boxes:
            ...
            # check class name
            box_cls_name = perception_3d_label_pb2_v32.ClassName.Name(
                box_info.class_name)
            ...
            if (box_cls_name.lower() in CLS
                ...
                box_motion_state = perception_3d_label_pb2_v32.MotionState.Name(
...
def check_seq_stability(tracking_id_dict, tracking_id_index_dict,
    ...
    for key, value in tracking_id_dict.items():
        ...
        cls = perception_3d_label_pb2_v32.ClassName.Name(
...
def check_global_position_moving(boxes, lidar_to_worlds, min_postion_diff):
    ...
    for i in range(len(position_x) - 1):
        position_x_diff.append(abs(position_x[i + 1] - position_x[i]))
        position_y_diff.append(abs(position_y[i + 1] - position_y[i]))
    mean_position_x_diff = np.mean(position_x_diff)
    mean_position_y_diff = np.mean(position_y_diff)
    if mean_position_x_diff < min_postion_diff and mean_position_y_diff < min_postion_diff:
        return False
    return True
        ...
        if len(value) > 2:
            for box in value:
                track_class_list.append(
                    perception_3d_label_pb2_v32.ClassName.Name(
...
def check_sequence(labeled_sequence, seq_result_dict, seq_error_dict):
    ...
    seq_result_dict["Info"][
        "TimePeriod"] = perception_3d_label_pb2_v32.TimePeriod.Name(
    ...
    for purpose in sequence_info.label_purpose:
        seq_result_dict["Info"]["LabelPurpose"].append(
            perception_3d_label_pb2_v32.LabelPurpose.Name(purpose))
    ...
    for batch_dir in sorted(os.listdir(labeled_data_dir)):
        ...
        for record_dir in sorted(os.listdir(batch_dir)):
            ...
            for seq_task_dir in sorted(
                ...
                labeled_sequence = perception_3d_label_pb2_v32.LabeledSequence()
