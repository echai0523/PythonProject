...
from wooey_utils.com.db_tool import get_data_db
...
class Dbclass():
    ...
    def initdb(self):
        engine = create_engine(
            f'postgresql+psycopg2://{self.db_setting["user"]}:{self.db_setting["pwd"]}@{self.db_setting["host"]}:5432/{self.dbname}',
            poolclass=NullPool,
...
# def get_protal_db():
#     db_setting = {"host": local_ip_if_linux(), "user": "postgres", "pwd": "appen_pe123"}
...
def get_image_json_info_from_db(imgUrl):
    rs = db.db_query(f'''select json_info from t_haomo_2d_json_info where imgurl='{imgUrl}';''')
...
class SINGEL_ROW_PROCESS():
    ...
    def parse_row_func(self):
        ...
        addition_info = remove_key_if_exists(
            info_dict,
            ["producer", "special", "edges", "lines", "objects"]  # 删除可能覆盖重要结果的字段
        )
