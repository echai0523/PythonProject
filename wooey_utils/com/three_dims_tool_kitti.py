...
class GEN_ManifestV3():
    def __init__(self,batch,group,total_frames,max_frames,
        ...
        self.raddar_dict = self.get_raddar_dict(raddar_url)
    ...
    def get_raddar_dict(self,raddar_url):
        if raddar_url is None:
            return None
        else:
            rs = self.session.get(raddar_url).json()
            return rs
    ...
    @staticmethod
    def template_manifest_json(tag,frames,cameraInfo,origins_list,client_json_lst,calibration=None,part_range_lst=None):
        manifest_template = {
            "format": {
                "type": "pcd",
                "pcd": {
    ...
    def get_manifest_json_task(self) :### 返回存储的路径和信息
        ...
        for i in range(0, self.total_frames, self.max_frames):
            ...
            ## 不需要manifest.json
            json_url = f'https://projecteng.oss-cn-shanghai.aliyuncs.com/{os.path.dirname(json_path)}/manifest.json'
            ...
            for t_idx in range(start,end):
                single_item = dict()
                single_item["lidar"] = {
                    "url": self.pcd_internalUrls[t_idx],
                    "externalUrl": self.pcd_urls[t_idx],
                }
                single_item["cameras"] = []
                for camera in self.image_folders:
                    single_item["cameras"].append({
                        "name":camera,
                        "url":self.img_internalUrls[camera][t_idx],
                        "externalUrl":self.img_urls[camera][t_idx],
                    })

                ### 加raddar
                jurlname  = os.path.basename(self.client_json_urls[t_idx]).split(".")[0]
                radar = self.raddar_dict[jurlname]
                radar_bin = array.array('f')
                radar_bin.extend(radar)
                url,_  = self.oss_obj.save_to_oss_url(radar_bin.tobytes(),suffix_type=".bin.radar")

                single_item["radar"]={
                    "url":url,
                    "externalUrl":url
                }
                frames_list.append(single_item)
    ...
    def gen_manifest_main(self):
        ...
        rs_df = pd.DataFrame(columns=["manifest_url","upload_batch","upload_group"])
        rs_df["manifest_url"] = json_url_list
